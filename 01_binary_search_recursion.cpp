...
vector<int>::iterator binary_search_recursion(vector<int>::iterator begin, vector<int>::iterator end, int target)
{
    if (begin == end)
        return begin;
    auto mid = begin + (end - begin) / 2;
    if (target < *mid)
        return binary_search_recursion(begin, mid, target);
    else if (target == *mid)
        return mid;
    else
        return binary_search_recursion(mid + 1, end, target);
}
...
int main()
{
    ...
    cout << '\n';
    cin >> target;
    result = binary_search_recursion(v.begin(), v.end(), target - 1);
    if (result != v.end())
    ...
    result = binary_search_recursion(v.begin(), v.end(), target);
    if (result != v.end())
